...
(function ($) {
	...
	if (parent.Modularity.Editor.Thickbox.postAction === 'import') {
		...
		$(document).on('click', '[data-modularity-action="import"]', function (e) {
			...
			let postId = $(e.target).closest('tr').attr('id');
			postId = postId.split('-')[1];

			const module = parent.Modularity.Editor.Module.isEditingModule();
	...
	if (parent.Modularity.Editor.Thickbox.postAction === 'import-widget') {
		...
		$(document).on('click', '[data-modularity-action="import"]', function (e) {
			...
			const widget = parent.Modularity.Helpers.Widget.isEditingWidget();
